                  ...
                  <div key={race.id} className="card" style={{marginBottom:8,borderLeft:`4px solid ${race.status==="finished"?C.muted:race.status==="closed"?C.red:raceBal===STARTING_BALANCE&&race.status==="upcoming"?C.red:raceBal===0?C.green:C.accent}`,cursor:race.status==="upcoming"?"pointer":"default",transition:"all .15s"}}
                    ...
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {/* Race name */}
                        <h3 className="cg" style={{fontSize:isMobile?14:22,fontWeight:700,marginBottom:1,lineHeight:1.2}}>{race.name}</h3>

                        {/* One-liner info row */}
                        <p className="sy" style={{fontSize:isMobile?10:13,color:C.soft,marginBottom:isMobile?2:4}}>{race.distance} · {active} runners{active<race.horses.length?` (${race.horses.length-active} scr)`:""}{fav?` · ⭐ ${fav.name} $${fav.winOdds.toFixed(1)}`:""}</p>

                        {/* Countdown + odds as of — compact */}
                        ...
                        {/* Player count */}
                        {race.status==="upcoming"&&accounts&&accounts.length>0&&(()=>{
                          ...
                          const allIn = playersWithBets===totalPlayers;
                          return(
                            <div style={{marginTop:3,display:"flex",alignItems:"center",gap:5}}>
                              <div style={{display:"flex",gap:2}}>
                                {accounts.map((a,i)=>{
                                  const hasBet = bets.some(b=>b.raceId===race.id&&b.playerId===a.id&&b.won===null);
                                  return <div key={a.id} style={{width:8,height:8,borderRadius:"50%",background:hasBet?C.green:"#e5e7eb",border:`1px solid ${hasBet?"#15803d":"#d1d5db"}`}}/>;
                                })}
                              </div>
                              <span className="sy" style={{fontSize:isMobile?9:11,color:allIn?C.green:C.soft,fontWeight:allIn?700:400}}>
                                {playersWithBets}/{totalPlayers} {allIn?"✓ All bets in":"players have bet"}
                              </span>
                            </div>
                          );
                        })()}

                        {/* Budget warning — show when < 10 mins and not fully spent */}
                        {race.status==="upcoming"&&account&&raceBal>0&&rb.length>0&&race.raceTime&&race.date&&(()=>{
                          const raceDateTime = new Date(`${race.date}T${race.raceTime}:00`);
                          const minsLeft = (raceDateTime - new Date()) / 60000;
                          if(minsLeft > 10 || minsLeft < 0) return null;
                          return(
                            <div style={{marginTop:4,padding:"4px 8px",borderRadius:6,background:"#fff3cd",border:"1px solid #ffc107",display:"flex",alignItems:"center",gap:5}}>
                              <span style={{fontSize:12}}>⚠️</span>
                              <span className="sy" style={{fontSize:isMobile?9:11,color:"#856404",fontWeight:700}}>
                                {fmt(raceBal)} unspent — betting closes in {Math.round(minsLeft)} min!
                              </span>
                            </div>
                          );
                        })()}

                        {/* Results — prominent on finished races */}
                        {race.status==="finished"&&race.result&&(
                          <div style={{marginTop:4,padding:"6px 10px",borderRadius:8,background:"rgba(21,128,61,.06)",border:`1px solid ${C.greenBd}`}}>
                            <div className="sy" style={{fontSize:isMobile?9:11,fontWeight:700,color:C.green,marginBottom:3}}>🏁 Result</div>
                            <div style={{display:"flex",gap:isMobile?8:12,flexWrap:"wrap"}}>
                              {["first","second","third","fourth"].map((k,i)=>{
                                const h=race.horses.find(x=>x.number===race.result[k]);
                                return h?<span key={k} className="sy" style={{fontSize:isMobile?9:11,color:i===0?C.accent:C.soft,fontWeight:i===0?700:400}}>{["1st","2nd","3rd","4th"][i]}: <strong>#{h.number} {h.name}</strong></span>:null;
                              })}
                            </div>
                          </div>
                        )}

                        {/* Who's backing what — compact chips on one line */}
                        {(race.status==="closed"||race.status==="finished")&&(()=>{
                          ...
                          const counts={};
                          raceBets.forEach(b=>{const n=b.horses[0];const h=race.horses.find(x=>x.number===n);if(h&&!h.scratched)counts[h.name]=(counts[h.name]||0)+1;});
                          const sorted=Object.entries(counts).sort(([,a],[,b])=>b-a).slice(0,4);
                          if(!sorted.length) return null;
                          return(
                            <div style={{marginTop:isMobile?4:8,display:"flex",flexWrap:"wrap",alignItems:"center",gap:4}}>
                              <span className="sy" style={{fontSize:isMobile?9:11,color:C.green,fontWeight:700}}>🏇</span>
                              {sorted.map(([name,count],idx)=>(
                                <span key={name} className="sy" style={{fontSize:isMobile?9:11,padding:isMobile?"2px 6px":"3px 9px",borderRadius:20,fontWeight:700,background:idx===0?C.green:"rgba(30,92,30,.08)",color:idx===0?"#fff":C.green,border:`1px solid ${C.green}`}}>
                                  {count}× {name}
                                </span>
                              ))}
                            </div>
                      ...
                      <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                        {race.status==="upcoming"&&account&&(
                          raceBal===0?(
                            <div style={{textAlign:"right"}}>
                              <button className="sy" style={{background:C.greenBg,border:`2px solid ${C.green}`,color:C.green,borderRadius:10,padding:isMobile?"8px 10px":"10px 16px",cursor:"pointer",fontWeight:700,fontSize:isMobile?12:13,display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}
                                onClick={e=>{e.stopPropagation();onSelect(race.id);}}>✅ Bets Confirmed</button>
                              {rb[0]?.placedAt&&<div className="sy" style={{fontSize:9,color:C.soft,marginTop:2,textAlign:"right"}}>
                                {new Date(rb[rb.length-1].placedAt).toLocaleTimeString("en-AU",{hour:"2-digit",minute:"2-digit"})}
                              </div>}
                            </div>
                          ):raceBal>0&&rb.length>0?(
                            <button className="sy" style={{background:"rgba(184,134,11,.1)",border:`2px solid ${C.gold}`,color:C.gold,borderRadius:10,padding:isMobile?"8px 10px":"10px 16px",cursor:"pointer",fontWeight:700,fontSize:isMobile?12:14,whiteSpace:"nowrap"}}
                              onClick={e=>{e.stopPropagation();onSelect(race.id);}}>⚡ {fmt(raceBal)}</button>
                          ):(
                            <button className="sy" style={{background:C.red,border:`2px solid ${C.red}`,color:"#fff",borderRadius:10,padding:isMobile?"10px 12px":"12px 20px",cursor:"pointer",fontWeight:900,fontSize:isMobile?13:15,animation:"pulse 1.5s infinite",boxShadow:"0 4px 14px rgba(185,28,28,.4)",whiteSpace:"nowrap"}}
                              onClick={e=>{e.stopPropagation();onSelect(race.id);}}>🚨 Bet Now!</button>
                          )
                        )}
                        {race.status==="finished"&&race.result&&(
                          <div className="sy" style={{fontSize:isMobile?9:10,textAlign:"right",lineHeight:1.4,color:C.soft}}>
                            {["first","second","third","fourth"].map((k,i)=>{
                              const h=race.horses.find(x=>x.number===race.result[k]);
                              return h?<div key={k} style={{fontWeight:i===0?700:400,color:i===0?C.accent:C.soft}}>{["1st","2nd","3rd","4th"][i]}: #{h.number}</div>:null;
                            })}
                          </div>
                        )}
                      </div>
                        ...
                        {/* Bets summary strip — merge EW win+place into one row */}
                        {rb.length>0&&(
                          <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
                            <div style={{display:"flex",flexWrap:"wrap",gap:4,flex:1}}>
                              {(()=>{
                                // Merge adjacent win+place bets on same horse into EW display
                                const displayed=[];
                                const ewPairs=new Set();
                                rb.forEach((b,idx)=>{
                                  if(ewPairs.has(b.id)) return;
                                  // Look for paired win/place bet on same horse placed at same time
                                  if(b.type==="win") {
                                    const pair=rb.find((x,xi)=>xi>idx&&x.type==="place"&&x.horses[0]===b.horses[0]&&Math.abs(new Date(x.placedAt)-new Date(b.placedAt))<5000);
                                    if(pair){ewPairs.add(pair.id);displayed.push({...b,type:"eachway",stake:b.stake,pairPayout:(b.payout||0)+(pair.payout||0),pairWon:b.won||pair.won,bothLost:b.won===false&&pair.won===false});return;}
                                  }
                                  if(b.type==="place"&&ewPairs.has(b.id)) return;
                                  displayed.push(b);
                                });
                                return displayed.map(b=>{
                                  const isEW=b.type==="eachway"&&b.pairPayout!==undefined;
                                  const def=BET_TYPES.find(t=>t.id===b.type)||{label:"Each Way"};
                                  const hasScratched=b.won===null&&b.horses.some(n=>race.horses.find(h=>h.number===n)?.scratched);
                                  const horseDisplay=b.horses.length===1?`#${b.horses[0]} ${race.horses.find(h=>h.number===b.horses[0])?.name||""}`:b.horses.map(n=>`#${n}`).join("-");
                                  const won=isEW?b.pairWon:b.won===true;
                                  const lost=isEW?b.bothLost:b.won===false;
                                  return(
                                    <div key={b.id} className="sy" style={{fontSize:isMobile?10:12,padding:isMobile?"3px 9px":"5px 12px",borderRadius:20,background:hasScratched?"#fff3cd":won?C.greenBg:lost?C.redBg:"#f4f5f4",border:`1px solid ${hasScratched?"#ffc107":won?C.greenBd:lost?C.redBd:C.border}`,color:hasScratched?"#856404":won?C.green:lost?C.red:C.text,fontWeight:600}}>
                                      {hasScratched?"⚠️ ":won?"✓ ":""}<strong>{isEW?"Each Way":def?.label}</strong> · {horseDisplay} · {fmt(b.stake)}{isEW&&b.pairWon?` → +${fmt(b.pairPayout)}`:lost?" · Lost":""}
                                    </div>
                                  );
                                });
                              })()}
  ...
  return (
    <div className="sr">
      ...
      <div style={{marginBottom:isMobile?8:16}}>
          ...
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
            ...
            {race.oddsAsOf&&(
              <div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",background:"rgba(184,134,11,0.12)",border:"1.5px solid rgba(184,134,11,0.4)",borderRadius:8,flexShrink:0}}>
                ...
                <div>
                  <div className="sy" style={{fontSize:9,textTransform:"uppercase",letterSpacing:".06em",color:C.gold,opacity:.7}}>Odds as of</div>
                  <div className="sy" style={{fontSize:15,fontWeight:800,color:C.gold,whiteSpace:"nowrap"}}>{race.oddsAsOf}</div>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Budget bar */}
        <div style={{padding:isMobile?"8px 12px":"12px 16px",borderRadius:8,background:raceBalance===0?"rgba(21,128,61,0.08)":raceBalance===STARTING_BALANCE?"rgba(185,28,28,0.07)":"rgba(30,92,30,0.06)",border:`2px solid ${raceBalance===0?C.greenBd:raceBalance===STARTING_BALANCE?C.redBd:C.accent}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
          <span className="sy" style={{fontSize:isMobile?12:14,fontWeight:700,color:raceBalance===0?C.green:raceBalance===STARTING_BALANCE?C.red:C.accent}}>
            {raceBalance===0?"✓ Full $24 spent!":raceBalance===STARTING_BALANCE?"⚠ No bets yet":`${fmt(raceBalance)} left to bet`}
          </span>
          <div className="cg" style={{fontSize:isMobile?14:20,fontWeight:800,color:raceBalance===0?C.green:C.accent,flexShrink:0}}>{fmt(raceBalance)}<span style={{fontSize:10,fontWeight:400,color:C.soft}}> / $24</span></div>
        </div>
      </div>

      {/* TAB-style bet type tabs — scrollable horizontal */}
      {isMobile&&(
        <div style={{display:"flex",gap:0,overflowX:"auto",marginBottom:10,borderBottom:`2px solid ${C.border}`,paddingBottom:0,WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
          {BET_TYPES.map(t=>(
            <button key={t.id} className="sy" style={{
              flexShrink:0,padding:"8px 14px",fontSize:12,fontWeight:betType===t.id?700:500,
              background:"transparent",border:"none",cursor:"pointer",
              color:betType===t.id?C.accent:C.soft,
              borderBottom:`2px solid ${betType===t.id?C.accent:"transparent"}`,
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 290px",gap:14,alignItems:"start"}}>
            ...
            return (
              <div key={h.number} style={{
                ...
                <div style={{display:"flex",alignItems:"center",gap:0}}>
                  ...
                  <div style={{flex:1,padding:isMobile?"6px 8px":"10px 12px",minWidth:0,cursor:scr?"default":"pointer"}}
                    ...
                    {h.form&&h.form.length>0&&(
                      <div style={{display:"flex",gap:2,marginTop:isMobile?2:4}}>
                        {h.form.map((f,fi)=>(<span key={fi} style={{width:isMobile?12:16,height:isMobile?12:16,borderRadius:2,background:formColor(f),display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobile?7:9,fontWeight:800,color:"#fff"}}>{f}</span>))}
                      </div>
                    )}
                    {/* Mobile: position pills for exotics inline */}
                  ...
                  {/* Right side buttons — desktop only for position pills, Win/Place for all */}
                  {!scr&&canShowBoxed&&!boxed&&!isMobile ? (
                    <div style={{display:"flex",flexDirection:"column",gap:5,padding:"12px 10px",flexShrink:0,alignItems:"flex-start"}}>
                      <div style={{display:"flex",gap:5}}>
                        {def.positions.map((pos,pi)=>{const isThis=(sel[pi]||[]).includes(h.number);return(<button key={pi} className="sy" style={{minWidth:44,padding:"8px 8px 6px",borderRadius:8,border:`2px solid ${isThis?"#1e5c1e":"#d1d5db"}`,background:isThis?C.accent:"#fff",color:isThis?"#fff":"#374151",cursor:"pointer",fontWeight:700,fontSize:13,textAlign:"center",transition:"all .13s",fontFamily:"inherit"}} onClick={e=>{e.stopPropagation();toggleHorse(pi,h.number);}}>{pos.label}</button>);})}
                      </div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:5,padding:"12px 10px",flexShrink:0,alignItems:"center"}}>
                      <button className="sy" style={{minWidth:72,padding:"10px 8px",borderRadius:8,border:`2px solid ${(sel[0]||[]).includes(h.number)?"#1e5c1e":"#d1d5db"}`,background:(sel[0]||[]).includes(h.number)?C.accent:"#fff",color:(sel[0]||[]).includes(h.number)?"#fff":"#374151",cursor:"pointer",fontWeight:700,fontSize:13,textAlign:"center",transition:"all .13s",fontFamily:"inherit"}} onClick={e=>{e.stopPropagation();toggleHorse(0,h.number);}}>{(sel[0]||[]).includes(h.number)?"✓ In":"Select"}</button>
                    ...
                    <div style={{display:"flex",gap:isMobile?4:6,padding:isMobile?"8px 6px":"12px 10px",flexShrink:0}}>
                      ...
                      <button className="sy" style={{
                        width:isMobile?56:78, padding:isMobile?"7px 0":"10px 0", borderRadius:isMobile?6:8,
                        border:`${isMobile?"1.5px":"2px"} solid ${winSel===h.number?"#1e5c1e":"rgba(30,92,30,.25)"}`,
                        ...
                        <div style={{fontSize:isMobile?13:16,fontWeight:800}}>${h.winOdds.toFixed(2)}</div>
                        <div style={{fontSize:isMobile?8:9,fontWeight:700,marginTop:1,opacity:.85}}>WIN</div>
                      </button>
                      {/* PLACE */}
                      <button className="sy" style={{
                        width:isMobile?56:78, padding:isMobile?"7px 0":"10px 0", borderRadius:isMobile?6:8,
                        border:`${isMobile?"1.5px":"2px"} solid ${placeSel===h.number?"#1d4ed8":"#d1d5db"}`,
                        ...
                        <div style={{fontSize:isMobile?13:16,fontWeight:800}}>${h.placeOdds.toFixed(2)}</div>
                        <div style={{fontSize:isMobile?8:9,fontWeight:700,marginTop:1,opacity:.85}}>PLACE</div>
                      </button>
                    </div>
                  ) : null}
                  {scr&&(
        ...
        {/* Mobile slide-up bet panel — appears when horse selected */}
        {isMobile&&(winSel||placeSel||(sel[0]||[]).length>0)&&(
          <>
            {/* Dim overlay */}
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.3)",zIndex:899}} onClick={()=>{setSel({});setWinSel(null);setPlaceSel(null);}}/>
            {/* Panel */}
            <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:900,background:"#fff",borderRadius:"20px 20px 0 0",boxShadow:"0 -8px 40px rgba(0,0,0,.2)",paddingBottom:"env(safe-area-inset-bottom,16px)"}}>
              {/* Drag handle */}
              <div style={{width:40,height:4,borderRadius:2,background:"#e5e7eb",margin:"12px auto 0"}}/>
              {/* Bet type tabs */}
              <div style={{display:"flex",overflowX:"auto",borderBottom:`1px solid ${C.border}`,padding:"0 16px",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
                {BET_TYPES.map(t=>(
                  <button key={t.id} className="sy" style={{flexShrink:0,padding:"10px 14px",fontSize:13,fontWeight:betType===t.id?700:500,background:"transparent",border:"none",cursor:"pointer",color:betType===t.id?C.accent:C.soft,borderBottom:`2.5px solid ${betType===t.id?C.accent:"transparent"}`,marginBottom:-1,whiteSpace:"nowrap"}}
                    onClick={()=>changeType(t.id)}>{t.label}</button>
                ))}
              </div>
              <div style={{padding:"14px 16px 4px"}}>
                {/* Selected horse */}
                {(()=>{
                  const selNum=winSel||placeSel||(sel[0]||[])[0];
                  const selH=race.horses.find(x=>x.number===selNum);
                  return selH?(
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"10px 12px",background:C.accentGlow,borderRadius:10}}>
                      {selH.silkUrl
                        ?<img src={selH.silkUrl} alt="" style={{width:36,height:36,objectFit:"contain",borderRadius:4,flexShrink:0}} onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}}/>
                        :null}
                      <div style={{width:36,height:36,borderRadius:"50%",background:silkCol(selH.number),display:selH.silkUrl?"none":"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#fff",flexShrink:0}}>{selH.number}</div>
                      <div style={{flex:1}}>
                        <div className="cg" style={{fontSize:15,fontWeight:700}}>{selH.name} <span style={{color:C.muted,fontWeight:400,fontSize:13}}>({selH.barrier||selH.number})</span></div>
                        <div className="sy" style={{fontSize:12,color:C.soft,marginTop:1}}>{betType==="win"?`Win $${selH.winOdds.toFixed(2)}`:betType==="place"?`Place $${selH.placeOdds.toFixed(2)}`:betType==="eachway"?`EW $${selH.winOdds.toFixed(2)}W / $${selH.placeOdds.toFixed(2)}P`:BET_TYPES.find(t=>t.id===betType)?.label}</div>
                      </div>
                      <div className="cg" style={{fontSize:22,fontWeight:800,color:C.accent}}>{fmt(raceBalance)}<span style={{fontSize:11,color:C.soft,fontWeight:400}}> left</span></div>
                    </div>
                  ):null;
                })()}
                {/* Stake input */}
                <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
                  <div style={{position:"relative",flex:1}}>
                    <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:C.muted,fontSize:16,fontWeight:600}}>$</span>
                    <input className="inp sy" type="number" min="0.5" step="0.5" placeholder="Stake" value={stakeStr} onChange={e=>setStakeStr(e.target.value)}
                      style={{paddingLeft:28,fontSize:16,padding:"12px 12px 12px 28px",width:"100%",borderRadius:10}}/>
                  </div>
                  <div style={{display:"flex",gap:6}}>
                    {[5,10,24].map(v=>(<button key={v} className="sy" style={{padding:"10px 12px",borderRadius:8,border:`1.5px solid ${C.border}`,background:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}} onClick={()=>setStakeStr(String(v))}>+${v}</button>))}
                  </div>
                </div>
                {betType==="eachway"&&stake>0&&<div className="sy" style={{fontSize:12,color:C.accent,marginBottom:8}}>EW = {fmt(stake)} Win + {fmt(stake)} Place = <strong>{fmt(totalCost)}</strong> total</div>}
                {totalCost>raceBalance&&stake>0&&<p className="sy" style={{fontSize:12,color:C.red,marginBottom:8}}>⚠ Exceeds your remaining budget of {fmt(raceBalance)}</p>}
                <div style={{display:"flex",gap:8,marginBottom:8}}>
                  <button className="btn btn-gold" disabled={!isReady()} onClick={()=>{handleAdd();}} style={{flex:1,padding:"14px",fontSize:15,fontWeight:700,borderRadius:12}}>
                    {!isReady()?(stake<=0?"Enter a stake":combos===0?"Select a horse":"Over budget"):`Add to Betslip — ${fmt(totalCost)}`}
                  </button>
                  <button className="sy" style={{padding:"14px 16px",borderRadius:12,border:`1.5px solid ${C.border}`,background:"#fff",fontSize:16,cursor:"pointer",color:C.soft,fontWeight:700}} onClick={()=>{setSel({});setWinSel(null);setPlaceSel(null);}}>✕</button>
                </div>
              </div>
            </div>
          </>
          ...
          <div style={{position:"sticky",top:70}}>
            <div className="card">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:14}}>
                {BET_TYPES.map(t=>(<button key={t.id} onClick={()=>changeType(t.id)} className="sy" style={{padding:"11px 14px",borderRadius:8,border:`2px solid ${betType===t.id?C.accent:C.border}`,background:betType===t.id?C.accentGlow:"#fff",color:C.text,cursor:"pointer",textAlign:"left",transition:"all .13s"}}><span style={{fontWeight:700,fontSize:14,color:betType===t.id?C.accent:C.text}}>{t.label}</span><span style={{color:C.soft,fontSize:13,marginLeft:6}}>— {t.desc}</span></button>))}
              </div>
...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
function AdminScreen({races, accounts, bets, adminUnlocked, setAdminUnlocked, onSettle, onScratch, onResetPin, onAddRace, onAddHorse, onDeleteRace, onEditRace, onEditHorse, seasonMessage, onSeasonMessage, toast, onLockRace}) {
  const w = useWindowWidth();
  const isMobile = w < 700;
