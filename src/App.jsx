              ...
              return(
                <div key={race.id} style={{
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:isMobile?12:18,padding:isFinished?(isMobile?"11px 14px":"13px 22px"):(isMobile?"14px 14px":"18px 22px")}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      {/* Venue + race + distance */}
                      <div style={{fontSize:isMobile?11:12,fontWeight:600,color:"#777",textTransform:"uppercase",letterSpacing:".05em",marginBottom:4}}>
                      ...
                      {isUpcoming&&account&&(
                        betPlaced?(
                          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                            <span style={{fontSize:13,fontWeight:800,color:"#15803d",background:"#dcfce7",borderRadius:20,padding:"4px 12px"}}>✓ Bet placed</span>
                            <button style={{fontSize:12,color:"#888",background:"none",border:"none",cursor:"pointer",padding:0,fontFamily:"inherit",textDecoration:"underline",textUnderlineOffset:2}} onClick={e=>{e.stopPropagation();onSelect(race.id);}}>Change</button>
                          </div>
                        ):myBet?(
                          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                            <span style={{fontSize:12,fontWeight:700,color:"#b45309",background:"#fef3c7",borderRadius:20,padding:"3px 11px",fontVariantNumeric:"tabular-nums"}}>${raceBal} left</span>
                            <button style={{fontSize:13,fontWeight:700,color:"#fff",background:"#d97706",border:"none",borderRadius:8,padding:"8px 16px",cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",boxShadow:"0 2px 6px rgba(217,119,6,.3)"}} onClick={e=>{e.stopPropagation();onSelect(race.id);}}>Add more</button>
                          </div>
                        ):(
                          <button style={{fontSize:14,fontWeight:700,color:"#fff",background:"#1a3a1a",border:"none",borderRadius:9,padding:"10px 20px",cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",boxShadow:"0 2px 8px rgba(26,58,26,.35)"}} onClick={e=>{e.stopPropagation();onSelect(race.id);}}>Bets Open</button>
                        )
                      )}
  ...
  return (
    <div className="fu">
      ...
      {(()=>{
        ...
        return(
          <div style={{position:"relative",overflow:"hidden",borderRadius:16,marginBottom:18,background:"linear-gradient(135deg,#1a3a1a 0%,#2d6a4f 100%)",boxShadow:"0 8px 26px rgba(15,32,16,.3)"}}>
            {/* Decorative laurel-style flourishes */}
            <div style={{position:"absolute",top:12,left:14,fontSize:isMobile?26:34,opacity:.18}}>🏆</div>
            <div style={{position:"absolute",bottom:8,right:16,fontSize:isMobile?36:46,opacity:.12,transform:"rotate(8deg)"}}>🐎</div>

            {/* Gold top band */}
            <div style={{background:"#dba53f",padding:"6px 0",textAlign:"center"}}>
              <span style={{fontSize:11,fontWeight:900,letterSpacing:".18em",textTransform:"uppercase",color:"#1a3a1a"}}>★ Bet of the Week ★</span>
            </div>

            <div style={{position:"relative",padding:isMobile?"20px 16px 22px":"26px 24px 28px",textAlign:"center"}}>
              <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.6)",marginBottom:10,letterSpacing:".04em"}}>{dateLabel}</div>

              {/* BIG name */}
              <div className="cg" style={{fontSize:isMobile?26:34,fontWeight:900,color:"#fff",lineHeight:1.15,marginBottom:4,wordBreak:"break-word"}}>{topWinNames||"Someone"}</div>
              <div style={{fontSize:isMobile?11:12,color:"rgba(255,255,255,.65)",marginBottom:16}}>
                {topWinDef?.label}{isBoxedStyle?(isTrueBox?" (Boxed)":" (Multi)"):""} on {topWinRace?.name}
              </div>

              {/* BIG payout */}
              <div style={{fontSize:isMobile?46:60,fontWeight:900,color:"#fcd34d",lineHeight:1,marginBottom:10,letterSpacing:"-.01em"}}>
                +{fmt(topWin.payout||0)}
              </div>
              {oddsHit>=5&&(
                <div style={{display:"inline-block",background:"rgba(255,255,255,.14)",borderRadius:20,padding:"3px 14px",fontSize:11,fontWeight:700,color:"#fff",marginBottom:16}}>
                  ${oddsHit.toFixed(1)} odds hit
                </div>
              )}

              {/* Runners strip */}
              <div style={{background:"rgba(0,0,0,.18)",borderRadius:10,padding:isMobile?"9px 14px":"10px 16px",display:"inline-block",maxWidth:"100%"}}>
                <div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,.5)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:2}}>Runners</div>
                <div style={{fontSize:12,fontWeight:700,color:"#fff",wordBreak:"break-word"}}>{horsesLabel}</div>
              </div>
            </div>
          </div>
        );
                    ...
                    <div style={{marginTop:14}}>
                      {resettleIds.has(race.id)&&(
                        <div style={{padding:"10px 14px",background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:8,marginBottom:14}}>
                          <p className="sy" style={{fontSize:12,fontWeight:700,color:"#92400e"}}>⚠️ Re-settle mode. Re-enter the result and dividends below, then hit Settle again — totals recalculate fresh each time, so this safely corrects any mistake or stuck save without double-paying anyone.</p>
                        </div>
                      )}
