                ...
                return (
                  <div key={name} style={{padding:"10px 14px",background:C.redBg,border:`1px solid ${C.redBd}`,borderRadius:8}}>
                    ...
                    {playerBets.map(b=>{
                      ...
                      return <div key={b.id} className="sy" style={{fontSize:13,color:C.soft}}>{td?.label} · #{b.horses.join(" → #")} · {fmt(b.stake)}</div>;
                    })}
                  </div>
                );
              })}
...
function PinPad({ value, onChange, maxLen=4 }) {
  ...
  return (
    <div style={{maxWidth:260,margin:"0 auto"}}>
      ...
      <p className="sy" style={{fontSize:12,color:C.muted,textAlign:"center",marginTop:12}}>You can also type your PIN using the keyboard</p>
    </div>
  );
}
              ...
              {step==="details"&&(
                <>
                  ...
                  <div style={{marginTop:14,padding:"10px 14px",background:"rgba(26,86,160,.06)",border:"1px solid rgba(26,86,160,.15)",borderRadius:8}}>
                    <p className="sy" style={{fontSize:13,color:C.soft}}>🎯 You <strong style={{color:C.accent}}>must spend $24.00</strong> on each individual Group 1 race - every race has its own $24 budget.</p>
                  </div>
                </>
              )}
              {step==="pin"&&(
    ...
    <div className="fu" style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 240px",gap:20,alignItems:"start"}}>
      <div>
        ...
        <div style={{marginBottom:isMobile?14:20}}>
          ...
          {account&&<p className="sy" style={{fontSize:isMobile?12:13,color:C.soft}}>Welcome back, <strong style={{color:C.text}}>{account.name.split(' ')[0]||account.name}</strong> 👋</p>}
        </div>
          ...
          return(
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
              {[
                ...
                <div key={label} style={{background:"#fff",borderRadius:12,padding:"10px 12px",border:`1px solid ${C.border}`,textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
                  <div className="sy" style={{fontSize:10,color:"#777",fontWeight:600,marginBottom:4,textTransform:"uppercase",letterSpacing:".04em"}}>{label}</div>
          ...
          <div key={date} style={{marginBottom:isMobile?16:24}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
              <div style={{background:isToday?"#1a3a1a":"#f0f7f0",borderRadius:10,padding:"6px 14px",flexShrink:0,border:isToday?"none":"1px solid #1a3a1a"}}>
                <span className="sy" style={{fontSize:isMobile?13:14,fontWeight:800,color:isToday?"#fff":"#000",whiteSpace:"nowrap"}}>
                  {isToday?"🏇 ":""}{dateLabel}
                </span>
              </div>
            ...
            {dayRaces.sort((a,b)=>(a.raceTime||"").localeCompare(b.raceTime||"")).map((race,raceIdx)=>{
              ...
              return(
                <div key={race.id} style={{
                  marginBottom:8,borderRadius:14,background:"#fff",overflow:"hidden",
                  border:hasScratched?"2px solid #f59e0b":urgent?"2px solid #dc2626":noBet?"2px solid #1a3a1a":"1px solid #e5e7eb",
                  boxShadow:urgent?"0 2px 16px rgba(220,38,38,.15)":noBet?"0 2px 12px rgba(26,58,26,.12)":"0 1px 4px rgba(0,0,0,.05)",
                  cursor:isUpcoming?"pointer":"default",transition:"all .18s",
                  animation:noBet&&!urgent?"subtleGlow 3s ease-in-out infinite":urgent?"pulse 1.5s infinite":"none",
                }}
                  onMouseEnter={e=>{if(!isMobile&&isUpcoming){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,.12)";}}}
                  onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=urgent?"0 2px 16px rgba(220,38,38,.15)":noBet?"0 2px 12px rgba(26,58,26,.12)":"0 1px 4px rgba(0,0,0,.05)";}}
                  onClick={()=>isUpcoming&&onSelect(race.id)}>

                  {/* Scratched banner */}
                  {hasScratched&&(
                    <div style={{padding:"6px 14px",background:"#fef3c7",borderBottom:"1px solid #f59e0b",display:"flex",gap:6,alignItems:"center"}}>
                      <span>⚠️</span>
                      <span className="sy" style={{fontSize:12,fontWeight:700,color:"#000"}}>Horse scratched — tap to update your bet</span>
                    </div>
                  )}
                  ...
                  <div style={{display:"flex",alignItems:"stretch"}}>
                    ...
                    {/* Left column — time + status pill */}
                    <div style={{
                      width:isMobile?64:72,flexShrink:0,
                      background:isFinished?"#f8f9fa":isClosed?"#fff0f0":"#1a3a1a",
                      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                      padding:"12px 4px",gap:4,
                    }}>
                      <span style={{fontSize:isMobile?15:17,fontWeight:900,letterSpacing:"-1px",
                        color:isFinished?"#9ca3af":isClosed?"#dc2626":"#fff",lineHeight:1}}>{timeLabel}</span>
                      <span style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:".04em",
                        color:isFinished?"#9ca3af":isClosed?"#f87171":"rgba(255,255,255,.65)"}}>
                        {isFinished?"Done":isClosed?"Locked":`Race ${raceIdx+1}`}
                      </span>
                      {/* Urgency indicator */}
                      {urgent&&(
                        <div style={{marginTop:2,background:"#dc2626",borderRadius:6,padding:"2px 6px"}}>
                          <span style={{fontSize:9,fontWeight:800,color:"#fff"}}>{minsUntil}m</span>
                        </div>
                      )}
                      {isUpcoming&&!urgent&&minsUntil!==null&&minsUntil>=0&&minsUntil<=60&&(
                        <div style={{marginTop:2,background:"rgba(255,255,255,.15)",borderRadius:6,padding:"2px 6px"}}>
                          <span style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,.9)"}}>{minsUntil}m</span>
                        </div>
                      )}
                    </div>

                    {/* Middle — all race info */}
                    <div style={{flex:1,minWidth:0,padding:isMobile?"10px 12px":"11px 16px"}}>

                      {/* Row 1: venue bold + grade badge */}
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>
                        <span className="sy" style={{fontSize:isMobile?11:12,fontWeight:800,color:"#1a3a1a",textTransform:"uppercase",letterSpacing:".04em"}}>{race.venue}</span>
                        {race.raceNum&&<span className="sy" style={{fontSize:isMobile?10:11,fontWeight:600,color:"#6b7280"}}>· R{race.raceNum.replace(/[^0-9]/g,"")}</span>}
                        {race.grade&&<span style={{fontSize:9,fontWeight:700,padding:"1px 6px",borderRadius:6,
                          background:race.grade==="Feature Race"?"#fef3c7":"#f0fdf4",
                          color:race.grade==="Feature Race"?"#92400e":"#166534",
                          border:`1px solid ${race.grade==="Feature Race"?"#fcd34d":"#bbf7d0"}`
                        }}>{race.grade}</span>}
                      </div>

                      {/* Row 2: race name — hero text */}
                      <div className="cg" style={{fontSize:isMobile?15:18,fontWeight:900,color:"#000",lineHeight:1.15,marginBottom:5}}>{race.name}</div>

                      {/* Row 3: distance · runners · fav all on one line */}
                      <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:isUpcoming||isFinished?5:0}}>
                        {race.distance&&<span className="sy" style={{fontSize:isMobile?11:12,fontWeight:600,color:"#000"}}>{race.distance}</span>}
                        {race.distance&&<span style={{color:"#d1d5db",fontSize:10}}>·</span>}
                        <span className="sy" style={{fontSize:isMobile?11:12,fontWeight:600,color:"#000"}}>
                          {active} runners{active<race.horses.length&&<span style={{color:"#dc2626"}}> ({race.horses.length-active} scr)</span>}
                        </span>
                        {fav&&fav.winOdds&&<>
                          <span style={{color:"#d1d5db",fontSize:10}}>·</span>
                          <span className="sy" style={{fontSize:isMobile?11:12,fontWeight:600,color:"#000"}}>⭐ <strong>{fav.name}</strong> <span style={{color:"#92400e",fontWeight:700}}>${fav.winOdds.toFixed(1)}</span></span>
                        </>}
                      </div>

                      {/* Row 4: countdown OR result */}
                      {isUpcoming&&race.raceTime&&(
                      ...
                      {isFinished&&race.result&&(
                        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:4}}>
                          {["first","second","third"].map((k,i)=>{
                            ...
                            return h?<span key={k} className="sy" style={{fontSize:11,fontWeight:i===0?800:600,color:"#000"}}>
                              <span style={{color:i===0?"#1a3a1a":"#6b7280",fontWeight:700}}>{["1st","2nd","3rd"][i]}</span> {h.name}
                            </span>:null;
                          })}
                        </div>
                      )}
                      ...
                      {/* Row 5: player dots */}
                      {isUpcoming&&accounts?.length>0&&(()=>{
                        const n=accounts.filter(a=>bets.some(b=>b.raceId===race.id&&b.playerId===a.id&&b.won===null)).length;
                        const allIn=n===accounts.length;
                        return(
                          <div style={{display:"flex",alignItems:"center",gap:4}}>
                            <div style={{display:"flex",gap:2,flexWrap:"wrap"}}>
                              {accounts.slice(0,isMobile?24:accounts.length).map(a=>{
                                const has=bets.some(b=>b.raceId===race.id&&b.playerId===a.id&&b.won===null);
                                return <div key={a.id} style={{width:6,height:6,borderRadius:"50%",background:has?"#1a3a1a":"#e5e7eb",flexShrink:0}}/>;
                              })}
                            </div>
                            <span className="sy" style={{fontSize:10,fontWeight:700,color:allIn?"#16a34a":"#000"}}>{n}/{accounts.length}{allIn?" · All in ✓":""}</span>
                          </div>
                        );
                      })()}

                      {/* Row 6: group picks (closed/finished) */}
                      {(isClosed||isFinished)&&(()=>{
                        const counts={};
                        ...
                        return(
                          <div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:2}}>
                            {sorted.map(([name,count],i)=>(
                              <span key={name} className="sy" style={{fontSize:10,padding:"1px 7px",borderRadius:20,fontWeight:700,
                                background:i===0?"#1a3a1a":"#f0f7f0",color:i===0?"#fff":"#000",
                                border:`1px solid ${i===0?"transparent":"#1a3a1a"}`}}>{count}× {name}</span>
                            ))}
                          </div>
                        );
                      })()}
                      ...
                      {/* My bet chips */}
                      {rb.length>0&&(
                        <div style={{marginTop:6,paddingTop:6,borderTop:"1px solid #f3f4f6",display:"flex",flexWrap:"wrap",gap:4}}>
                          {(()=>{
                            ...
                            return displayed.map(b=>{
                              ...
                              return(
                                <span key={b.id} className="sy" style={{fontSize:11,padding:"3px 9px",borderRadius:20,fontWeight:700,
                                  background:hasScr?"#fef3c7":won?"#dcfce7":lost?"#fee2e2":"#f0f7f0",
                                  border:`1.5px solid ${hasScr?"#f59e0b":won?"#16a34a":lost?"#dc2626":"#1a3a1a"}`,
                                  color:"#000"}}>
                                  {won?"✓ ":lost?"✗ ":hasScr?"⚠️ ":""}<strong>{isEW?"EW":def2?.label}</strong> {hn?.name}{won?` +${fmt(isEW?b.pairPayout:b.payout)}`:lost?" L":""}
                                </span>
                              );
                            });
                          })()}
                          {isUpcoming&&rb.some(b=>b.won===null)&&(
                            <span className="sy" style={{fontSize:11,padding:"3px 9px",borderRadius:20,border:"1.5px solid #dc2626",background:"#fee2e2",color:"#000",cursor:"pointer",fontWeight:700}}
                              onClick={e=>{e.stopPropagation();onSelect(race.id);}}>Edit →</span>
                          )}
                        </div>
                      )}
                    </div>
                    ...
                    {/* Right — CTA */}
                    {isUpcoming&&account&&(
                      <div style={{flexShrink:0,display:"flex",alignItems:"center",padding:isMobile?"10px 10px":"12px 14px"}}>
                        {betPlaced?(
                          <div style={{textAlign:"center",padding:"8px 10px",borderRadius:10,background:"#dcfce7",border:"2px solid #16a34a",minWidth:52}}>
                            <div style={{fontSize:18}}>✅</div>
                            <div className="sy" style={{fontSize:10,fontWeight:800,color:"#000",marginTop:1}}>In</div>
                          </div>
                        ):rb.length>0?(
                          <div style={{textAlign:"center",padding:"8px 10px",borderRadius:10,background:"#fef3c7",border:"2px solid #f59e0b",cursor:"pointer",minWidth:52}} onClick={e=>{e.stopPropagation();onSelect(race.id);}}>
                            <div style={{fontSize:18}}>⚡</div>
                            <div className="sy" style={{fontSize:10,fontWeight:800,color:"#000",marginTop:1}}>${raceBal}</div>
                          </div>
                        ):(
                          <div style={{textAlign:"center",padding:"10px 12px",borderRadius:10,background:"#1a3a1a",cursor:"pointer",minWidth:56,
                            boxShadow:"0 4px 14px rgba(26,58,26,.4)",animation:"pulse 2s infinite"}} onClick={e=>{e.stopPropagation();onSelect(race.id);}}>
                            <div style={{fontSize:18}}>🏇</div>
                            <div className="sy" style={{fontSize:11,fontWeight:900,color:"#fff",marginTop:1}}>Bet</div>
                            <div className="sy" style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.75)"}}>$24</div>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    return(
                      <div key={a.id} style={{padding:"7px 14px",background:isMe?"rgba(26,58,26,.05)":"transparent",borderLeft:`3px solid ${i<3?mc[i]:isMe?"#1a3a1a":"transparent"}`}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                          ...
                          <span className="sy" style={{fontSize:13,fontWeight:700,color:a.totalWon>0?C.green:a.totalWon===0?"#9ca3af":C.red,flexShrink:0}}>{a.totalWon>0?"+":a.totalWon<0?"-":""}{fmt(Math.abs(a.totalWon))}</span>
                        </div>
              ...
              {[
                ...
                <div key={title} style={{display:"flex",gap:10,marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${C.border}`}}>
                  ...
                  <div>
                    ...
                    <div className="sy" style={{fontSize:12,color:"#444",lineHeight:1.5}}>{desc}</div>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="sr">
      ...
      <div style={{marginBottom:isMobile?8:16}}>
        <button className="btn btn-ghost sy" style={{marginBottom:12,fontSize:13,padding:"8px 16px",fontWeight:600}} onClick={onBack}>← All Races</button>
        ...
        <div style={{marginBottom:isMobile?8:14,padding:isMobile?"12px 14px":"16px 20px",background:"#fff",borderRadius:14,border:`1px solid ${C.border}`,boxShadow:"0 1px 6px rgba(0,0,0,.06)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
            ...
            <div style={{textAlign:"right",flexShrink:0}}>
              <div className="cg" style={{fontSize:isMobile?20:26,fontWeight:900,color:raceBalance===0?C.green:raceBalance===STARTING_BALANCE?C.red:C.accent,lineHeight:1}}>{raceBalance===0?"✓ All in!":fmt(raceBalance)}</div>
              <div className="sy" style={{fontSize:12,color:"#374151",fontWeight:600}}>of $24 left</div>
            </div>
          </div>
          {/* Budget progress bar */}
                ...
                {myBets.map(b=>{
                  ...
                  return(
                    <div key={b.id} style={{display:"flex",alignItems:"center",gap:6,padding:isMobile?"8px 12px":"6px 10px",borderRadius:8,background:b.won===true?C.greenBg:b.won===false?C.redBg:"rgba(26,58,26,.06)",border:`1.5px solid ${b.won===true?C.greenBd:b.won===false?C.redBd:"#1a3a1a"}`}}>
                      ...
                      {horse&&<span className="sy" style={{fontSize:13,color:C.soft}}>#{horse.number} {horse.name}</span>}
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 290px",gap:14,alignItems:"start"}}>
        ...
        <div>
          ...
          {!isMobile&&(
            <div style={{display:"flex",justifyContent:"flex-end",gap:8,padding:"0 10px 6px",borderBottom:`1px solid ${C.border}`,marginBottom:6}}>
              <span className="sy" style={{fontSize:12,textTransform:"uppercase",letterSpacing:".1em",color:"#555",width:72,textAlign:"center",fontWeight:700}}>WIN</span>
              <span className="sy" style={{fontSize:12,textTransform:"uppercase",letterSpacing:".1em",color:"#555",width:80,textAlign:"center",fontWeight:700}}>PLACE</span>
            </div>
          )}
                  ...
                  <div style={{flex:1,padding:isMobile?"8px 8px":"10px 12px",minWidth:0,cursor:scr?"default":"pointer"}}
                    ...
                    {!isMobile&&!scr&&canShowBoxed&&boxed&&(
                      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}>
                        <span className="sy" style={{fontSize:12,color:C.accent}}>${h.winOdds.toFixed(2)}W · ${h.placeOdds.toFixed(2)}P</span>
                      </div>
                    )}
                  </div>
                      ...
                      canShowBoxed&&!boxed ? (
                        ...
                        <div style={{display:"flex",gap:5,padding:"10px 8px",flexShrink:0}}>
                          <button className="sy" style={{width:72,padding:"9px 0",borderRadius:7,border:`2px solid ${winSel===h.number?"#1a3a1a":"rgba(30,92,30,.3)"}`,background:winSel===h.number?"#1a3a1a":"rgba(30,92,30,.06)",color:winSel===h.number?"#fff":"#1a3a1a",cursor:"pointer",textAlign:"center",fontFamily:"inherit"}}
                            ...
                            <div style={{fontSize:12,fontWeight:700,opacity:.8,letterSpacing:".04em"}}>WIN</div>
                          </button>
                          <button className="sy" style={{width:72,padding:"9px 0",borderRadius:7,border:`2px solid ${placeSel===h.number?"#1d4ed8":"#d1d5db"}`,background:placeSel===h.number?"#1d4ed8":"#f8f9fb",color:placeSel===h.number?"#fff":"#4b5563",cursor:"pointer",textAlign:"center",fontFamily:"inherit"}}
                            ...
                            <div style={{fontSize:12,fontWeight:700,opacity:.8,letterSpacing:".04em"}}>PLACE</div>
                          </button>
                        </div>
                      )
                    )
        ...
        {isMobile&&showBetPanel&&(
          <>
            ...
            <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:900,background:"#fff",borderRadius:"16px 16px 0 0",boxShadow:"0 -4px 30px rgba(0,0,0,.2)",paddingBottom:"env(safe-area-inset-bottom,16px)",maxHeight:"82vh",display:"flex",flexDirection:"column"}}>

              {/* Handle + close */}
              <div style={{flexShrink:0,padding:"8px 14px 0"}}>
                <div style={{width:32,height:3,borderRadius:2,background:"#e5e7eb",margin:"0 auto 8px"}}/>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span className="sy" style={{fontSize:13,fontWeight:700,color:"#111"}}>Place a Bet</span>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span className="sy" style={{fontSize:12,color:C.accent,fontWeight:700}}>{fmt(raceBalance)} left</span>
                    <button style={{width:26,height:26,borderRadius:"50%",background:"#f3f4f6",border:"none",cursor:"pointer",fontSize:14,color:"#444",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>{setShowBetPanel(false);setSel({});setWinSel(null);setPlaceSel(null);setStakeStr("");}}>✕</button>
                  </div>
                </div>

                {/* Bet type - scrollable pills */}
                <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:2,WebkitOverflowScrolling:"touch",scrollbarWidth:"none",msOverflowStyle:"none",marginBottom:8}}>
                  {BET_TYPES.map(t=>(
                    <button key={t.id} className="sy" style={{flexShrink:0,padding:"6px 12px",borderRadius:20,border:`2px solid ${betType===t.id?"#1a3a1a":"#e5e7eb"}`,background:betType===t.id?"#1a3a1a":"#fff",color:betType===t.id?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}
              ...
              <div style={{flex:1,overflowY:"auto",padding:"0 14px"}}>
                ...
                {/* Win/Place/EW - show selected horse or picker */}
                {(betType==="win"||betType==="place"||betType==="eachway")&&(
                        ...
                        return h?(
                          <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:"#f0fdf4",borderRadius:10,border:"2px solid #bbf7d0",marginBottom:10}}>
                            ...
                            <div style={{flex:1}}>
                              ...
                              <div className="sy" style={{fontSize:12,color:"#4b7c4b"}}>Win ${h.winOdds.toFixed(2)} · Place ${h.placeOdds.toFixed(2)}</div>
                            </div>
                            <button style={{background:"none",border:"none",fontSize:18,color:"#555",cursor:"pointer",lineHeight:1}} onClick={()=>{setWinSel(null);setPlaceSel(null);setSel({});}}>×</button>
                          </div>
                        ):null;
                      })()
                    ):(
                      <div>
                        <p className="sy" style={{fontSize:12,fontWeight:700,color:"#374151",marginBottom:6}}>Select a horse</p>
                        <div style={{display:"flex",flexDirection:"column",gap:5}}>
                          {race.horses.filter(h=>!h.scratched).map(h=>(
                            <button key={h.number} className="sy" style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:9,border:`1.5px solid ${(winSel===h.number||placeSel===h.number)?"#1a3a1a":"#e5e7eb"}`,background:(winSel===h.number||placeSel===h.number)?"#f0fdf4":"#fafafa",cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}
                              onClick={()=>{
                  ...
                  <div style={{marginBottom:10}}>
                    {canShowBoxed&&(
                      <div style={{display:"flex",gap:6,marginBottom:8}}>
                        <button className="sy" style={{flex:1,padding:"8px",borderRadius:9,border:`2px solid ${!boxed?"#1a3a1a":"#e5e7eb"}`,background:!boxed?"#1a3a1a":"#fff",color:!boxed?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>{setBoxed(false);setSel({});}}>Unboxed</button>
                        <button className="sy" style={{flex:1,padding:"8px",borderRadius:9,border:`2px solid ${boxed?"#1a3a1a":"#e5e7eb"}`,background:boxed?"#1a3a1a":"#fff",color:boxed?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>{setBoxed(true);setSel({});}}>Boxed</button>
                      </div>
                    )}
                    <p className="sy" style={{fontSize:12,fontWeight:700,color:"#374151",marginBottom:6}}>{boxed?`Select ${numPositions}+ horses`:"Tap position for each horse"}</p>
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      {race.horses.filter(h=>!h.scratched).map(h=>{
                        if(boxed){
                          ...
                          return(
                            <button key={h.number} className="sy" style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:9,border:`1.5px solid ${inSel?"#1a3a1a":"#e5e7eb"}`,background:inSel?"#f0fdf4":"#fafafa",cursor:"pointer",textAlign:"left",fontFamily:"inherit"}} onClick={()=>toggleHorse(0,h.number)}>
                              ...
                              <span className="sy" style={{fontSize:12,fontWeight:700,color:"#1a3a1a"}}>${h.winOdds.toFixed(2)}</span>
                              {inSel&&<span style={{fontSize:14,color:"#1a3a1a",fontWeight:800}}>✓</span>}
                            </button>
                          );
                        }
                        ...
                        return(
                          <div key={h.number} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 10px",borderRadius:9,border:`1.5px solid ${myPos.length?"#1a3a1a":"#e5e7eb"}`,background:myPos.length?"#f0fdf4":"#fafafa"}}>
                            ...
                            <div style={{display:"flex",gap:3}}>
                              {def.positions.map((pos,pi)=>{
                                const isThis=(sel[pi]||[]).includes(h.number);
                                return <button key={pi} className="sy" style={{minWidth:38,height:32,borderRadius:7,border:`2px solid ${isThis?"#1a3a1a":"#d1d5db"}`,background:isThis?"#1a3a1a":"#fff",color:isThis?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>toggleHorse(pi,h.number)}>{pos.label}</button>;
                              })}
                            </div>
                          </div>
                        );
                    ...
                    {combos>0&&<div style={{marginTop:8,padding:"8px 10px",background:"#f0fdf4",borderRadius:8,border:"1px solid #bbf7d0"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span className="sy" style={{fontSize:12,color:"#1a3a1a",fontWeight:700}}>✓ {combos} combination{combos!==1?"s":""}</span>
                        {combos>1&&<span className="sy" style={{fontSize:12,color:"#1a3a1a",fontWeight:700}}>{flexiPct}% flexi · {fmt(unitStake)} each</span>}
                      </div>
                    </div>}
                  </div>
                )}
                ...
                {/* Stake */}
                <div style={{borderTop:"1px solid #f3f4f6",paddingTop:10,marginBottom:6}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span className="sy" style={{fontSize:12,fontWeight:700,color:"#374151"}}>Stake</span>
                    <span className="sy" style={{fontSize:12,color:"#555"}}>{fmt(raceBalance)} remaining</span>
                  </div>
                  <div style={{position:"relative",marginBottom:8}}>
                    ...
                    <input className="inp sy" type="number" min="0.5" step="0.5" placeholder="0.00" value={stakeStr} onChange={e=>setStakeStr(e.target.value)}
                      style={{paddingLeft:28,fontSize:20,fontWeight:700,padding:"11px 12px 11px 28px",width:"100%",borderRadius:10,border:"2px solid #e5e7eb"}}/>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:8}}>
                    {[2,5,10,raceBalance].filter((v,i,a)=>v>0&&a.indexOf(v)===i).slice(0,4).map(v=>(
                      <button key={v} className="sy" style={{padding:"11px 4px",borderRadius:9,minHeight:44,border:`2px solid ${stakeStr===String(v)?"#1a3a1a":"#e5e7eb"}`,background:stakeStr===String(v)?"#1a3a1a":"#fff",color:stakeStr===String(v)?"#fff":"#374151",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>setStakeStr(String(v))}>
                        ${v}
                      </button>
                    ))}
                  </div>
                  {betType==="eachway"&&stake>0&&<p className="sy" style={{fontSize:12,color:"#444",marginBottom:6}}>EW = {fmt(stake)} + {fmt(stake)} = <strong>{fmt(totalCost)}</strong></p>}
                  {totalCost>raceBalance&&stake>0&&<p className="sy" style={{fontSize:12,color:"#dc2626",marginBottom:6,fontWeight:600}}>⚠ Only {fmt(raceBalance)} remaining</p>}
                </div>
              </div>
              ...
              <div style={{flexShrink:0,padding:"10px 14px"}}>
                <button className="sy" disabled={!isReady()} onClick={()=>{handleAdd();setShowBetPanel(false);setSel({});setWinSel(null);setPlaceSel(null);setStakeStr("");}}
                  style={{width:"100%",padding:"14px",borderRadius:12,background:isReady()?"#1a3a1a":"#e5e7eb",color:isReady()?"#fff":"#9ca3af",fontSize:15,fontWeight:800,border:"none",cursor:isReady()?"pointer":"not-allowed",fontFamily:"inherit"}}>
          ...
          <div style={{position:"sticky",top:70,display:"flex",flexDirection:"column",gap:12}}>
              ...
              <div style={{padding:"12px"}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {BET_TYPES.map(t=>(
                    <button key={t.id} onClick={()=>changeType(t.id)} className="sy" style={{
                      ...
                      border:`1.5px solid ${betType===t.id?"#1a3a1a":C.border}`,
                      background:betType===t.id?"#1a3a1a":"#fafafa",
                      color:betType===t.id?"#fff":"#374151",
                ...
                {canShowBoxed&&(
                  <div style={{marginTop:10}}>
                    ...
                    <p className="sy" style={{fontSize:12,color:C.soft,marginTop:6,lineHeight:1.5}}>{boxed?`Select ${numPositions}+ horses - any order`:`Use position buttons on each horse`}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{background:"#fff",borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,.05)"}}>
              ...
              <div style={{padding:"14px 16px",borderBottom:`1px solid #f3f4f6`,minHeight:64}}>
                {(betType==="win"||betType==="place"||betType==="eachway")?(
                  !winSel&&!placeSel
                    ?<p className="sy" style={{fontSize:12,color:"#555",fontStyle:"italic",paddingTop:4}}>← Tap WIN or PLACE on a horse</p>
                    :(()=>{
                      const n=winSel||placeSel;
                      const h=race.horses.find(x=>x.number===n);
                      return h?(
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <div style={{flex:1}}>
                            <div className="sy" style={{fontSize:14,fontWeight:700,color:"#111"}}>{h.name} <span style={{color:"#555",fontWeight:400,fontSize:12}}>({h.barrier||h.number})</span></div>
                            <div className="sy" style={{fontSize:12,color:C.soft,marginTop:1}}>
                              Win <strong style={{color:"#1a3a1a"}}>${h.winOdds.toFixed(2)}</strong> · Place <strong style={{color:"#1d4ed8"}}>${h.placeOdds.toFixed(2)}</strong>
                            </div>
                          </div>
                          <button style={{background:"none",border:"none",fontSize:18,color:"#555",cursor:"pointer",padding:"4px"}} onClick={()=>{setWinSel(null);setPlaceSel(null);setSel({});}}>×</button>
                        </div>
                      ):null;
                    })()
                ):(
                  boxed?(
                    <div>
                      <div style={{display:"flex",flexDirection:"column",gap:5}}>
                        {(sel[0]||[]).length===0
                          ?<span className="sy" style={{fontSize:12,color:"#555",fontStyle:"italic"}}>← Select on each horse</span>
                      ...
                      {def.positions.map((pos,pi)=>{
                        ...
                        return(
                          <div key={pi} style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                            ...
                            {posHorses.length===0
                              ?<span className="sy" style={{fontSize:12,color:"#555",fontStyle:"italic"}}>-</span>
                              :posHorses.map(h=>(
              ...
              <div style={{padding:"14px 16px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <p className="sy" style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",color:"#555",margin:0}}>Stake</p>
                  <p className="sy" style={{fontSize:12,color:C.soft,margin:0}}>{fmt(raceBalance)} remaining</p>
                </div>

                {/* Big stake input */}
                <div style={{position:"relative",marginBottom:10}}>
                  <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:20,fontWeight:700,color:"#555"}}>$</span>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:12}}>
                  {[2,5,10,raceBalance].filter((v,i,a)=>a.indexOf(v)===i&&v>0).slice(0,4).map(v=>(
                    <button key={v} className="sy" style={{padding:"9px 4px",borderRadius:8,border:`1.5px solid ${stakeStr===String(v)?"#1a3a1a":"#e5e7eb"}`,background:stakeStr===String(v)?"#1a3a1a":"#fff",color:stakeStr===String(v)?"#fff":"#374151",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}
                ...
                {stake>0&&combos>0&&(
                  <div style={{padding:"10px 14px",background:"#f8f9fa",borderRadius:10,marginBottom:12,border:"1px solid #e5e7eb"}}>
                    {combos>1&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      <span className="sy" style={{fontSize:13,color:C.soft}}>Combinations</span>
                    ...
                    {(betType==="trifecta"||betType==="firstfour"||betType==="exacta"||betType==="quinella")&&combos>1&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      <span className="sy" style={{fontSize:13,color:C.soft}}>Unit stake</span>
                    ...
                    {(betType==="trifecta"||betType==="firstfour"||betType==="exacta"||betType==="quinella")&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      <span className="sy" style={{fontSize:13,color:C.soft}}>Flexi %</span>
                    ...
                    {betType==="eachway"&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      <span className="sy" style={{fontSize:13,color:C.soft}}>Win + Place</span>
            ...
            {myBets.length>0&&(
              <div className="card" style={{padding:"16px"}}>
                <p className="sy" style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".1em",marginBottom:10}}>Your Bets on This Race</p>
                      ...
                      <div key={b.id} style={{padding:"10px 12px",background:b.won===true?C.greenBg:b.won===false?C.redBg:"#f8fffe",border:`1.5px solid ${b.won===true?C.greenBd:b.won===false?C.redBd:C.greenBd}`,borderRadius:8}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:3}}>
                          <div>
                            ...
                            <span className="sy" style={{fontSize:13,color:C.soft}}> · {fmt(b.stake)}</span>
                          </div>
                        ...
                        <div className="sy" style={{fontSize:13,color:C.soft}}>{horses}</div>
                        {canCancel&&(
                ...
                return(
                  <div key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"10px 12px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8}}>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div className="sy" style={{fontSize:13,color:C.soft}}>{def?.label} · #{b.horses.join(" → #")}</div>
                  ...
                  {isExp&&(
                    <div style={{padding:"14px 16px 16px",background:"#f8fffe",borderBottom:`1px solid ${C.border}`,borderLeft:`3px solid ${leftBorderCol}`}}>
                      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:14,marginBottom:12}}>
                        {/* Form dots */}
                        <div>
                          <div className="sy" style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:8,textTransform:"uppercase",letterSpacing:".06em"}}>
                        ...
                        {/* Best win */}
                        <div>
                          <div className="sy" style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:8,textTransform:"uppercase",letterSpacing:".06em"}}>🌟 Best Win</div>
                          {bestWin?(
                            <>
                              ...
                              <div className="sy" style={{fontSize:12,color:C.soft}}>{BET_TYPES.find(t=>t.id===bestWin.type)?.label} · {bestWinRace?.name}</div>
                            </>
                          ):<span className="sy" style={{fontSize:12,color:C.muted,fontStyle:"italic"}}>No wins yet</span>}
                        </div>
                      </div>
                      {/* Stats grid */}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:0}}>
                {[
                  ...
                  <div key={l} style={{display:"grid",gridTemplateColumns:"1fr 80px 1fr",gap:8,padding:"10px 0",borderBottom:`1px solid ${C.border}`,alignItems:"center"}}>
                    ...
                    <span className="sy" style={{fontSize:11,color:C.muted,textAlign:"center",fontWeight:600}}>{l}</span>
            ...
            return(
              <div key={race.id} className="surface" style={{borderLeft:`3px solid ${race.status==="finished"?C.muted:bal>0?C.accent:C.green}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:5}}>
                  ...
                  <div className="sy" style={{fontSize:12,color:race.status==="finished"?C.muted:bal===0?C.green:C.accent,fontWeight:600}}>
                ...
                {race.status==="upcoming"&&bal>0&&(
                  <div className="sy" style={{fontSize:12,color:C.red,fontWeight:600}}>⚠ Must spend {fmt(bal)} more</div>
                )}
              </div>
            );
          })}
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:8,marginBottom:16}}>
          {bestWin&&(
            <div style={{background:"#f0fdf4",border:`1px solid ${C.greenBd}`,borderRadius:12,padding:"14px 16px"}}>
              ...
              <div className="sy" style={{fontSize:13,color:C.muted,marginTop:1}}>#{bestWin.horses.join(" → #")}</div>
            </div>
          )}
              ...
              return(
                <div className="card" style={{marginBottom:12}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr",gap:10}}>
                    ...
                    <div>
                      <div className="sy" style={{fontSize:11,color:C.muted,marginBottom:8,fontWeight:600}}>Betting efficiency</div>
                      <div style={{display:"flex",flexDirection:"column",gap:7}}>
                        {[
                          ...
                          <div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
                            <span className="sy" style={{fontSize:12,color:C.soft}}>{l}</span>
          ...
          <div style={{marginBottom:24}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
              <div className="card" style={{textAlign:"center",padding:"18px 12px"}}>
                <div className="sy" style={{fontSize:10,fontWeight:600,color:"#6b7280",marginBottom:10,textTransform:"uppercase",letterSpacing:".06em"}}>Win Rate</div>
                {(()=>{
              ...
              <div className="card" style={{textAlign:"center",padding:"18px 12px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                <div className="sy" style={{fontSize:10,fontWeight:600,color:"#6b7280",marginBottom:8,textTransform:"uppercase",letterSpacing:".06em"}}>Streak</div>
                ...
                <div key={race.id} className="card" style={{borderLeft:`3px solid ${bal===0?C.green:C.accent}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                    <div>
                      ...
                      <div className="sy" style={{fontSize:13,color:C.soft}}>{race.venue} · {new Date(race.date).toLocaleDateString("en-AU",{day:"numeric",month:"short"})}</div>
                    </div>
                        ...
                        return(
                          <div key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:7}}>
                            <div>
                              ...
                              <span className="sy" style={{fontSize:13,color:C.soft}}> · #{b.horses.join(" → #")}</span>
                            </div>
              ...
              return(
                <div key={race.id} className="card" style={{borderLeft:`3px solid ${raceProfit>=0?C.green:C.red}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:rb.length>0?8:0}}>
                    <div>
                      <div className="cg" style={{fontSize:16,fontWeight:700}}>{race.name}</div>
                      <div className="sy" style={{fontSize:13,color:C.soft}}>{race.venue} · {new Date(race.date).toLocaleDateString("en-AU",{day:"numeric",month:"short",year:"numeric"})}</div>
                      {winner&&<div className="sy" style={{fontSize:12,marginTop:2,color:C.accent,fontWeight:600}}>🥇 {winner.name}</div>}
                    </div>
                    <div style={{textAlign:"right",flexShrink:0}}>
                      <div className="sy" style={{fontSize:13,color:C.soft}}>Your result</div>
      ...
      {adminTab === "races" && (
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <p className="sy" style={{fontSize:13,color:C.soft}}>Click horses into finishing order, enter TAB dividends, then settle.</p>
                  ...
                  {bulkPreview.map((h,i)=>(
                    <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 10px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,fontSize:12,gap:8,alignItems:"center"}}>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
                        {h.form&&h.form.length>0&&<span className="sy" style={{fontSize:13,color:C.soft}}>{h.form.slice(-6).join("-")}</span>}
